...
/**
 * POST /admin/profiles/extract - Extract style or persona from sample text
 * Uses cloud LLM (llama-3.1-70b) for extraction
 */
adminProfileRoutes.post('/extract', async (c) => {
  try {
    ...
    if (!text || text.length < 200) {
      return c.json({ error: 'Text must be at least 200 characters' }, 400);
    }
    ...
    console.log(`[ProfileExtraction] Type: ${type}, Model: ${modelId}, Environment: ${environment}`);
