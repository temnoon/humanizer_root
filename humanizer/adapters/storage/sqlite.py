...
Implements storage protocols using SQLite + SQLAlchemy async.

This module provides the mobile/desktop storage backend for:
...
class SQLiteStorageBase:
    ...
    def __init__(self, db_path: Optional[str] = None):
        ...
        Args:
            db_path: Path to SQLite database file (default: ./humanizer.db)
        """
        if db_path is None:
            db_path = "./humanizer.db"
...
class SQLiteConversationStorage(SQLiteStorageBase):
    """
    SQLite implementation of ConversationStorage protocol.
    ...
    async def search_conversations(
        ...
        """
        Search conversations by text (simple LIKE search).
        ...
        async with self.session_maker() as session:
            ...
            stmt = (
                select(AgentConversation)
                .where(AgentConversation.user_id == user_id)
                .where(AgentConversation.title.like(search_pattern))
                .order_by(AgentConversation.updated_at.desc())
                .limit(limit)
            )
            result = await session.execute(stmt)
...
class SQLiteDocumentStorage(SQLiteStorageBase):
    """
    ...
    async def list_documents(
        ...
        async with self.session_maker() as session:
            ...
            user_documents = [
                ...
                if doc.custom_metadata and doc.custom_metadata.get("user_id") == str(user_id)
            ]
    ...
    async def search_documents(
        ...
        """
        Search documents by text (simple LIKE search).
        ...
        async with self.session_maker() as session:
            ...
            # Fetch all documents matching text search
            stmt = (
                select(Document)
                .where(
...
class SQLiteTransformationStorage(SQLiteStorageBase):
    """
    SQLite implementation of TransformationStorage protocol.
