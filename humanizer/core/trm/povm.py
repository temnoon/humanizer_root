...
class POVMPack:
    ...
    def __post_init__(self):
        ...
        if diff > 0.01:  # Tolerance
            # Proper POVM normalization using matrix decomposition
            # We want: Σ E_i = I, currently: Σ E_i = total
            # Solution: Transform E_i_new = L^{-1} @ E_i @ L^{-T}
            # where total = L @ L.T (Cholesky decomposition)
            # Then: Σ E_i_new = L^{-1} @ (Σ E_i) @ L^{-T} = L^{-1} @ total @ L^{-T} = I
            ...
            try:
                ...
                # Transform each B_i → B_i_new = L^{-1} @ B_i
                # This gives E_i_new = B_i_new @ B_i_new.T = L^{-1} @ B_i @ B_i.T @ L^{-T} = L^{-1} @ E_i @ L^{-T}
                for op in self.operators:
                    op.B = L_inv @ op.B
                ...
                # If Cholesky fails (total not PSD), fall back to uniform scaling
                # This is less accurate but more robust
                total_trace = np.trace(total)
                if total_trace > 0:
                    scale_factor = np.sqrt(self.rank / total_trace)
                    for op in self.operators:
                    ...
                    if diff > 0.1:
                        ...
                        for op in self.operators:
                            new_total += op.E
                        new_diff = np.linalg.norm(new_total - identity)
                        print(f"⚠ POVM '{self.name}' scaled (fallback): {diff:.4f} → {new_diff:.4f}")
...
def create_random_povm_pack(
    ...
    # Proper normalization to ensure Σ E_i = I
    # Use matrix decomposition: total = L @ L.T, then transform B_i → L^{-1} @ B_i
    ...
    try:
        ...
        for axis, B_i in temp_operators:
            B_i_normalized = L_inv @ B_i
            operators.append(POVMOperator(name=axis, B=B_i_normalized))

    except np.linalg.LinAlgError:
        # Fallback to trace-based scaling if Cholesky fails
