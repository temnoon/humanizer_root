 ...
 * Ported from narrative-studio: ClaudeParser.ts
 */

import * as fs from 'fs';
import * as path from 'path';
import * as crypto from 'crypto';
import type { Conversation, ClaudeExport, ClaudeChatMessage, ConversationMapping, ConversationNode } from './types.js';
import { readJSON, parseISOTimestamp } from './utils.js';
...
export class ClaudeParser {
  ...
  private convertToOpenAIFormat(claudeConv: ClaudeExport): Conversation {
    ...
    const mappingValues = Object.values(mapping) as Array<{
      parent?: string;
      children: string[];
      id: string;
    }>;
   ...
   */
  private buildMappingFromMessages(messages: ClaudeChatMessage[]): ConversationMapping {
    ...
    let previousNodeId: string = rootId;

    // Convert each message to a node
    for (const msg of messages) {
      const nodeId = msg.uuid || crypto.randomUUID();
   ...
   */
  private findLastNode(mapping: ConversationMapping, nodeId: string): string {
  ...
  static async detectFormat(extractedDir: string): Promise<boolean> {
    ...
    const data = readJSON<unknown>(conversationsFile);
    ...
    const firstConv = data[0] as Record<string, unknown>;
