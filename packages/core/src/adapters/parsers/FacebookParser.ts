 ...
 * Ported from narrative-studio: FacebookParser.ts
 */
...
export class FacebookParser {
   ...
   * Facebook messages are linear (no branching)
   */
  private buildMapping(messages: FacebookMessage[], conversationId: string): ConversationMapping {
    ...
    sortedMessages.forEach((fbMsg, index) => {
      ...
      const nextNodeId =
        index < sortedMessages.length - 1 ? `${conversationId}_msg_${index + 1}` : undefined;
  ...
  private convertMessage(fbMsg: FacebookMessage, messageId: string): Message {
    ...
    const message: Message = {
      id: messageId,
      author: author,
      create_time: Math.floor(fbMsg.timestamp_ms / 1000),
